...
## ダウンロード
[ダウンロードはこちら](https://github.com/roshi-hiraiwa/evaluate_model/releases/latest)
...
1. 学習・予測を実行する<br>
   `2_classification_loo.bat`を実行して、計算を完了させてください。（これによって`b3_practice2/target/result.csv`が生成されます）

2. 評価を実行する<br>
   `3_evaluate_model.bat`をダブルクリックして実行してください。コマンドプロンプトが立ち上がり、モデルの評価指標（混同行列、正解率、適合率、再現率、F1値）が表示されます。<br>
...
## 注意事項
以下のファイルは、評価算出の元データとして使用します。内容を書き換えると正しい結果が得られません。<br>
